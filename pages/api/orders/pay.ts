...
import { IPaypalOrderResponse } from '../../../interface';
...
const payOrder = async (req: NextApiRequest, res: NextApiResponse<Data>) => {
  ...
  const { data } = await axios.get<IPaypalOrderResponse>(URL, {
    headers: {
  ...
  if(dbOrder.total !== Number(data.purchase_units[0].amount.value)){
    await db.disconnect();
    return res.status(400).json({ message: 'Los montos de PayPal y esta orden no son iguales' });
  }
